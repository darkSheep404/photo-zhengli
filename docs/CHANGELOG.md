...
1. **已排除相册折叠**：将已排除相册移至常用相册之后的默认折叠区，降低其在主列表中的视觉干扰，仍支持展开后单独整理和长按恢复。
2. **组合排序开关**：将“常用优先”和名称排序改为可同时启用的独立开关；名称排序可循环切换 A-Z、Z-A 和关闭状态。
3. **完成弹窗间距**：为“继续清理下一批”和“返回首页”按钮增加操作区间距，避免触控区域视觉粘连。
4. **相册大小与排序**：相册列表显示图片总大小；新增照片数量、占用大小的升序/降序排序，并与名称排序互斥。
5. **完成反馈**：清理完成弹窗改用烟花图标，并显示本次移动和保留的分类照片数量。
6. **媒体管理授权**：在 Android 12+ 设置页增加可选的 `MANAGE_MEDIA` 特殊权限入口，基于系统实际授权状态显示；不支持的设备不会显示该项。

---

## v0.1.1 相册与整理流程优化（2026-08-25）

### 相册管理
1. **长按确认排除**：长按相册仅弹出确认弹窗；确认后才排除或恢复，点击相册始终可直接进入单独整理。
2. **常用相册**：星标相册可置顶；支持“常用优先 / 名称 A-Z”切换；移动照片面板将常用相册独立置顶显示。

### 整理状态与统计
3. **放弃整理确认**：审核页底部“返回首页”改为“放弃本次整理”，二次确认后清空未完成决策，防止下次整理继承旧的删除、移动或保留标记。
4. **完成记录收口**：确认删除和无删除完成共用同一条统计写入路径，使用完成前的删除、移动、保留数量快照，避免记录出现错误的零值或重复写入。

---

## 会话交接文档（2026-08-25）

- 新增 `docs/SESSION_HANDOVER.md`：记录当前架构、已实现功能、真机验证重点、已知风险、构建发布流程及新会话建议起点。
